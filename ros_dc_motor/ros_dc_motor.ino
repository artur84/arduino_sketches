...
PID PID_R (&Input, &Output, &Setpoint, consKp, consKi, consKd, DIRECT);
PID PID_L (&Input, &Output, &Setpoint, consKp, consKi, consKd, DIRECT);
 ...
 */
void
move_motor (int motor, float speed)
{
...
void
move_robot (float linearx, float angularz)
{
...
//Twist callback
void
cmd_vel_cb (const geometry_msgs::Twist& cmd_msg)
{
...
loop ()
{
  ...
  if (!(millis () % 10))
    {	//Control the motors every 10ms aprox.
      move_robot (global_linx, global_angz);
    }

  if (!(millis () % 3000))
    {	//Say I'm ok once in a while
